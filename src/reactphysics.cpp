...
CollisionCallback::ContactPoint* contact_pair_get_contact_point(uint32 index, const CollisionCallback::ContactPair* pair) {
	if (!pair) return nullptr;
	if (index >= pair->getNbContactPoints()) return nullptr;
	return new CollisionCallback::ContactPoint(pair->getContactPoint(index));
}
...
void RegisterPhysicsWorldAndCallbacks(asIScriptEngine* engine) {
	engine->RegisterObjectType("physics_contact_point", 0, asOBJ_REF | asOBJ_NOCOUNT);
	...
	engine->RegisterObjectMethod("physics_contact_pair", "physics_contact_point@ get_contact_point(uint index) const", asFUNCTION(contact_pair_get_contact_point), asCALL_CDECL_OBJLAST);
